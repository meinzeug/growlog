...
import { SkeletonCard, SkeletonChart, SkeletonPanel } from '../components/ui/Skeletons';
    ...
    useEffect(() => {
        const fetchStatsAndChart = async () => {
            try {
                ...
                setError(t('dashboard_load_error') || 'Failed to load dashboard data');
            } finally {
        ...
        const fetchPlants = async () => {
            // ... (keep existing fetchPlants logic, maybe add error handling too)
            try {
                ...
                console.error("Dashboard plants load failed", e);
                // We might not want to block the whole dashboard for plants failure, but maybe show a toast
            } finally {
                setLoadingStates(prev => ({ ...prev, plants: false }));
            }
        };
        ...
        <div className="min-h-full">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-full">
                ...
                <div className="lg:col-span-8 space-y-8 flex flex-col">
                        ...
                        <div>
                            {authLoading ? (
                                ...
                                <h1 className="text-2xl lg:text-3xl font-bold text-slate-900 font-sans">
                                    {t('hello')} {user?.email?.split('@')[0] || t('grower')}
                                </h1>
                            )}
                    ...
                    {error && (
                        <div className="bg-red-50 text-red-600 p-4 rounded-xl border border-red-100 flex justify-between items-center mb-6">
                            ...
                            <button
                                ...
                                className="px-3 py-1 bg-white border border-red-200 rounded-lg text-sm font-medium hover:bg-red-50 transition-colors shadow-sm"
                            >
                                {t('retry') || 'Retry'}
                            </button>
                        </div>
